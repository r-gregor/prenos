...
void make_long_date(char *file_name, char *tmstmp) {
	...
	int yr = gmtime(&finfo.st_mtime)->tm_year + 1900;
	int mnt = gmtime(&finfo.st_mtime)->tm_mon + 1;   // month, range 0 to 11 -->+1
	int dy = gmtime(&finfo.st_mtime)->tm_mday;
